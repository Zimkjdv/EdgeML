...
<template>
  <main class="page-shell">
    ...
    <section v-else>
      <el-card v-if="selectedTrainedModel" class="result-card"><template #header>相關係數（Pearson R）</template><el-descriptions :column="2" border><el-descriptions-item label="驗證相關係數（Validation Pearson R）">{{ formatNumber(selectedTrainedModel.validation_metrics?.pearson_r) }}</el-descriptions-item><el-descriptions-item label="測試相關係數（Test Pearson R）">{{ formatNumber(selectedTrainedModel.test_metrics?.pearson_r) }}</el-descriptions-item></el-descriptions></el-card>
      <el-card v-if="selectedTrainedModel" class="result-card unified-model-detail"><template #header><div class="result-heading"><span>{{ selectedTrainedModel.name }}：模型詳細資訊與評估</span><span><el-button v-if="selectedTrainedModel.status === 'draft'" type="primary" @click="publish(selectedTrainedModel)">發布至 Prediction</el-button><el-button v-else type="success" @click="loadPublished(selectedTrainedModel)">載入到 Prediction</el-button></span></div></template><el-descriptions :column="2" border><el-descriptions-item label="演算法">{{ selectedTrainedModel.algorithm }}</el-descriptions-item><el-descriptions-item label="目標欄位">{{ selectedTrainedModel.target_column }}</el-descriptions-item><el-descriptions-item label="訓練特徵" :span="2">{{ selectedTrainedModel.feature_columns?.join('、') }}</el-descriptions-item><el-descriptions-item><template #label>決定係數（Validation R²） <el-tooltip placement="top"><template #content>小於 0.6：模型只具一般參考價值。<br />0.6～0.9：通常具高參考價值。<br />大於 0.9：通常代表非常強的參考價值。</template><el-icon><QuestionFilled /></el-icon></el-tooltip></template>{{ formatNumber(selectedTrainedModel.validation_metrics?.r2) }}</el-descriptions-item><el-descriptions-item label="Validation MAE">{{ formatNumber(selectedTrainedModel.validation_metrics?.mae) }}</el-descriptions-item><el-descriptions-item label="Validation RMSE">{{ formatNumber(selectedTrainedModel.validation_metrics?.rmse) }}</el-descriptions-item><el-descriptions-item label="Validation RMSE 標準差">{{ formatNumber(selectedTrainedModel.validation_metrics?.rmse_std) }}</el-descriptions-item><el-descriptions-item label="外部測試 MAE">{{ formatNumber(selectedTrainedModel.test_metrics?.mae) }}</el-descriptions-item><el-descriptions-item label="外部測試 MAPE (%)">{{ formatNumber(selectedTrainedModel.test_metrics?.mape) }}</el-descriptions-item><el-descriptions-item label="外部測試 RMSE">{{ formatNumber(selectedTrainedModel.test_metrics?.rmse) }}</el-descriptions-item><el-descriptions-item label="外部測試 NRMSE">{{ formatNumber(selectedTrainedModel.test_metrics?.nrmse) }}</el-descriptions-item><el-descriptions-item label="最大誤差">{{ formatNumber(selectedTrainedModel.test_metrics?.max_error) }}</el-descriptions-item><el-descriptions-item label="目標平均">{{ formatNumber(selectedTrainedModel.test_metrics?.target_mean) }}</el-descriptions-item><el-descriptions-item label="相關係數（Pearson R）">{{ formatNumber(selectedTrainedModel.test_metrics?.pearson_r) }}</el-descriptions-item><el-descriptions-item><template #label>決定係數（Test R² Score） <el-tooltip placement="top"><template #content>小於 0.6：模型只具一般參考價值。<br />0.6～0.9：通常具高參考價值。<br />大於 0.9：通常代表非常強的參考價值。</template><el-icon><QuestionFilled /></el-icon></el-tooltip></template>{{ formatNumber(selectedTrainedModel.test_metrics?.r2) }}</el-descriptions-item></el-descriptions><p class="helper">未選外部測試集時，外部測試指標會顯示「—」。</p></el-card>
