...
type TrainedModel = { id: string; name: string; completed_at: string; target_column: string; algorithm: string; problem_type: string; validation_rmse?: number | null; validation_r2?: number | null; test_rmse?: number | null; test_r2?: number | null; status: 'draft' | 'published'; feature_columns?: string[]; evaluation_version?: string; validation_metrics?: Record<string, number | null>; test_metrics?: Record<string, number | null>; settings?: Record<string, unknown>; manifest?: Record<string, unknown> }
...
const randomForestParams = ref<{ n_estimators?: number; min_samples_leaf?: number; max_depth?: number; min_samples_split?: number; max_leaf_nodes?: number }>({})
...
const formatNumber = (value?: number | null) => value === undefined || value === null || !Number.isFinite(value) ? '—' : Number(value).toLocaleString('zh-TW', { maximumFractionDigits: 4 })
  ...
  try {
    ...
    const job = await api<{ id: string }>('/api/training/jobs', {
      ...
      body: JSON.stringify({
        ...
        dimension_reduction: training.value.dimensionReduction, svd_components: training.value.svdComponents, hyperparameters: training.value.algorithm.includes('xgboost') ? xgbParams.value : training.value.algorithm.includes('gradient_boosting') ? gradientBoostingParams.value : training.value.algorithm.includes('random_forest') ? randomForestParams.value : training.value.algorithm === 'logistic_regression' ? logisticRegressionParams.value : {},
      }),
    })
...
const pollTrainingJob = async (id: string): Promise<void> => {
  const job = await api<{ status: string; progress: number; message: string; result_model_id?: string; error?: string }>(`/api/training/jobs/${id}`)
  trainingProgress.value = job.progress; trainingMessage.value = job.message
  if (job.status === 'completed') {
    trainingResult.value = await api<TrainedModel>(`/api/trained-models/${job.result_model_id}`)
    selectedTrainedModel.value = trainingResult.value
    trainedModelRename.value = trainingResult.value.name
    await refreshTrainedModels(); activePage.value = 'trained'; ElMessage.success('模型訓練完成。'); return
  }
  if (job.status === 'failed' || job.status === 'cancelled') throw new Error(job.error ?? (job.status === 'cancelled' ? t('queueCancelled') : '訓練失敗。'))
  await new Promise(resolve => window.setTimeout(resolve, 700)); return pollTrainingJob(id)
}
  ...
  <div :class="locale === 'en' ? 'locale-en' : 'locale-zh'">
    ...
    <section v-else-if="activePage === 'training'">
      <el-card class="workspace"><template #header>{{ training.problemType === 'classification' ? t('classification') : t('regression') }} {{ t('training') }}</template><el-alert :title="training.problemType === 'classification' ? t('classificationNotice') : t('trainingNotice')" type="info" :closable="false" show-icon class="bottom-gap" /><el-form label-position="top">
       ...
       <el-card v-if="selectedDataset && training.algorithm.includes('random_forest')" class="result-card"><template #header>{{ t('rfParameters') }}</template><el-row :gutter="12"><el-col :span="8"><el-form-item label="Iter Number"><el-input-number v-model="randomForestParams.n_estimators" :min="1" /></el-form-item></el-col><el-col :span="8"><el-form-item label="Min Sample Leaf"><el-input-number v-model="randomForestParams.min_samples_leaf" :min="1" /></el-form-item></el-col><el-col :span="8"><el-form-item label="Max Depth"><el-input-number v-model="randomForestParams.max_depth" :min="1" /></el-form-item></el-col><el-col :span="8"><el-form-item label="Min Sample Split"><el-input-number v-model="randomForestParams.min_samples_split" :min="2" /></el-form-item></el-col><el-col :span="8"><el-form-item label="Max Leaf Node"><el-input-number v-model="randomForestParams.max_leaf_nodes" :min="2" /></el-form-item></el-col></el-row><p class="helper">{{ t('rfHint') }}</p></el-card>
       ...
       <el-card v-if="selectedDataset && training.algorithm === 'logistic_regression'" class="result-card"><template #header>{{ t('logisticParameters') }}</template><el-row :gutter="12"><el-col :span="8"><el-form-item label="penalty"><el-select v-model="logisticRegressionParams.penalty" clearable :placeholder="locale === 'zh-TW' ? '預設 L2' : 'Default L2'"><el-option value="l1" label="L1" /><el-option value="l2" label="L2" /></el-select></el-form-item></el-col><el-col :span="8"><el-form-item label="solver"><el-select v-model="logisticRegressionParams.solver" clearable :placeholder="locale === 'zh-TW' ? '預設 lbfgs' : 'Default lbfgs'"><el-option value="lbfgs" label="lbfgs" /><el-option value="liblinear" label="liblinear" /><el-option value="saga" label="saga" /></el-select></el-form-item></el-col><el-col :span="8"><el-form-item label="C"><el-input-number v-model="logisticRegressionParams.C" :min="0.0001" :step="0.1" /></el-form-item></el-col></el-row><p class="helper">{{ t('logisticHint') }}</p></el-card>
        <el-card v-if="selectedDataset && !training.algorithm.includes('xgboost') && !training.algorithm.includes('gradient_boosting') && !training.algorithm.includes('random_forest') && training.algorithm !== 'logistic_regression'" class="result-card"><template #header>{{ training.algorithm }} {{ t('hyperParameters') }}</template><el-empty :description="t('hyperParametersReserved')" :image-size="72" /></el-card>
    ...
    <section v-else-if="activePage === 'trained'" class="trained-models-page">
      <FeatureImportance v-if="selectedTrainedModel" :key="selectedTrainedModel.id" :model-id="selectedTrainedModel.id" />
      ...
      <el-card v-if="selectedTrainedModel" class="result-card unified-model-detail"><template #header><div class="result-heading"><span>{{ selectedTrainedModel.name }}：模型詳細資訊與評估</span><span><el-button v-if="selectedTrainedModel.status === 'draft'" type="primary" class="publish-button" @click="publish(selectedTrainedModel)">發布至 Prediction</el-button><el-button v-else type="success" class="load-button" @click="loadPublished(selectedTrainedModel)">載入到 Prediction</el-button></span></div></template><el-descriptions :column="2" border><el-descriptions-item label="演算法">{{ selectedTrainedModel.algorithm }}</el-descriptions-item><el-descriptions-item label="目標欄位">{{ selectedTrainedModel.target_column }}</el-descriptions-item><el-descriptions-item :span="2"><template #label>訓練特徵（{{ selectedTrainedModel.feature_columns?.length ?? 0 }} 個）</template><div class="feature-list"><el-tag v-for="feature in selectedTrainedModel.feature_columns" :key="feature" size="small" effect="plain">{{ feature }}</el-tag></div></el-descriptions-item><template v-if="selectedTrainedModel.problem_type === 'regression'"><el-descriptions-item><template #label>驗證 決定係數 R² <el-tooltip content="R² = 1 − SSE/SST。最佳為 1，可為負值；常數目標的整體 R² 無法定義。各折平均不等於整體 OOF R²，參考價值須依領域與測試資料判斷。" placement="top"><el-icon><QuestionFilled /></el-icon></el-tooltip></template>{{ formatNumber(selectedTrainedModel.validation_metrics?.r2) }}</el-descriptions-item><el-descriptions-item><template #label>測試 決定係數 R² <el-tooltip content="R² = 1 − SSE/SST。最佳為 1，可為負值；常數目標的整體 R² 無法定義。各折平均不等於整體 OOF R²，參考價值須依領域與測試資料判斷。" placement="top"><el-icon><QuestionFilled /></el-icon></el-tooltip></template>{{ formatNumber(selectedTrainedModel.test_metrics?.r2) }}</el-descriptions-item><el-descriptions-item label="驗證 相關係數 Pearson R">{{ formatNumber(selectedTrainedModel.validation_metrics?.pearson_r) }}</el-descriptions-item><el-descriptions-item label="測試 相關係數 Pearson R">{{ formatNumber(selectedTrainedModel.test_metrics?.pearson_r) }}</el-descriptions-item><el-descriptions-item label="驗證 MAE">{{ formatNumber(selectedTrainedModel.validation_metrics?.mae) }}</el-descriptions-item><el-descriptions-item label="測試 MAE">{{ formatNumber(selectedTrainedModel.test_metrics?.mae) }}</el-descriptions-item><el-descriptions-item label="驗證 MAPE (%)">{{ formatNumber(selectedTrainedModel.validation_metrics?.mape) }}</el-descriptions-item><el-descriptions-item label="測試 MAPE (%)">{{ formatNumber(selectedTrainedModel.test_metrics?.mape) }}</el-descriptions-item><el-descriptions-item label="驗證 RMSE">{{ formatNumber(selectedTrainedModel.validation_metrics?.rmse) }}</el-descriptions-item><el-descriptions-item label="測試 RMSE">{{ formatNumber(selectedTrainedModel.test_metrics?.rmse) }}</el-descriptions-item><el-descriptions-item label="驗證 NRMSE（以範圍正規化）">{{ formatNumber(selectedTrainedModel.validation_metrics?.nrmse) }}</el-descriptions-item><el-descriptions-item label="測試 NRMSE（以範圍正規化）">{{ formatNumber(selectedTrainedModel.test_metrics?.nrmse) }}</el-descriptions-item><el-descriptions-item label="驗證 最大誤差">{{ formatNumber(selectedTrainedModel.validation_metrics?.max_error) }}</el-descriptions-item><el-descriptions-item label="測試 最大誤差">{{ formatNumber(selectedTrainedModel.test_metrics?.max_error) }}</el-descriptions-item><el-descriptions-item label="驗證 目標平均">{{ formatNumber(selectedTrainedModel.validation_metrics?.target_mean) }}</el-descriptions-item><el-descriptions-item label="測試 目標平均">{{ formatNumber(selectedTrainedModel.test_metrics?.target_mean) }}</el-descriptions-item><el-descriptions-item label="各折 RMSE 平均">{{ formatNumber(selectedTrainedModel.validation_metrics?.cv_rmse_mean) }}</el-descriptions-item><el-descriptions-item label="各折 RMSE 標準差">{{ formatNumber(selectedTrainedModel.validation_metrics?.rmse_std) }}</el-descriptions-item><el-descriptions-item label="各折 MAE 平均">{{ formatNumber(selectedTrainedModel.validation_metrics?.cv_mae_mean) }}</el-descriptions-item><el-descriptions-item><template #label>各折 R² 平均 <el-tooltip content="R² = 1 − SSE/SST。最佳為 1，可為負值；常數目標的整體 R² 無法定義。各折平均不等於整體 OOF R²，參考價值須依領域與測試資料判斷。" placement="top"><el-icon><QuestionFilled /></el-icon></el-tooltip></template>{{ formatNumber(selectedTrainedModel.validation_metrics?.cv_r2_mean) }}</el-descriptions-item></template></el-descriptions><p v-if="selectedTrainedModel.problem_type === 'regression'" class="helper">新版驗證指標以整體折外預測（OOF）計算；各折平均與 RMSE 標準差另列。NRMSE = RMSE ÷（真實目標最大值 − 最小值），不是百分比。真實目標含零值時 MAPE 顯示「—」；接近零時 MAPE 可能偏大。常數目標的 R²、NRMSE，以及常數輸入或不足兩筆的 Pearson R 無法定義，顯示「—」。R² 可為負值；Pearson R 高不代表預測誤差小，須配合 MAE、RMSE 與領域要求判讀。</p><p v-if="selectedTrainedModel.problem_type === 'regression' &amp;&amp; selectedTrainedModel.evaluation_version !== 'oof-v2'" class="helper">此為舊版模型紀錄，驗證分數未自動重算。請重新訓練以取得新版 OOF 指標；外部測試可重新執行評估。</p><p class="helper">未提供測試資料或指標無法計算時顯示「—」。</p></el-card>
