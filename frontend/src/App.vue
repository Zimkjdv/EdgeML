  ...
  try {
    ...
    const job = await api<{ id: string }>('/api/training/jobs', {
      ...
      body: JSON.stringify({
        ...
        dimension_reduction: training.value.dimensionReduction, svd_components: training.value.svdComponents, hyperparameters: training.value.algorithm.includes('xgboost') ? xgbParams.value : {},
      }),
    })
  ...
  <main class="page-shell" :class="locale === 'en' ? 'locale-en' : 'locale-zh'">
    ...
    <section v-else-if="activePage === 'training'">
      <el-card class="workspace"><template #header>{{ training.problemType === 'classification' ? t('classification') : t('regression') }} {{ t('training') }}</template><el-alert :title="training.problemType === 'classification' ? t('classificationNotice') : t('trainingNotice')" type="info" :closable="false" show-icon class="bottom-gap" /><el-form label-position="top">
        ...
        <el-form-item :label="t('algorithm')"><el-radio-group v-model="training.algorithm"><template v-if="training.problemType === 'regression'"><el-radio value="random_forest">Random Forest</el-radio><el-radio value="gradient_boosting">Gradient Boosting</el-radio><el-radio value="xgboost">XGBoost</el-radio><el-radio value="adaboost">AdaBoost</el-radio><el-radio value="ridge">Ridge</el-radio></template><template v-else><el-radio value="random_forest_classifier">Random Forest</el-radio><el-radio value="gradient_boosting_classifier">Gradient Boosting</el-radio><el-radio value="xgboost_classifier">XGBoost</el-radio><el-radio value="adaboost_classifier">AdaBoost</el-radio></template></el-radio-group></el-form-item>
       ...
       <el-card v-if="selectedDataset && !training.algorithm.includes('xgboost')" class="result-card"><template #header>{{ training.algorithm }} {{ t('hyperParameters') }}</template><el-empty :description="t('hyperParametersReserved')" :image-size="72" /></el-card>
