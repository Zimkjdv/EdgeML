...
- **v0.7.3 (in progress):** bounded retries with configurable attempt limits, exponential backoff, dead-letter routing, and graceful worker shutdown are implemented. A Queue Operations UI for worker status, queue depth, retry attempts, dead-letter jobs, and controlled retry/requeue/cancel actions remains planned, along with worker capacity controls and runtime integration tests.
