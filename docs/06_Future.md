...
- **Reliability backlog:** Redis persistence, shared-volume worker recovery, durable published packages, Registry transactions, recoverable dead-letter replay, explicit anonymous mode, numeric dtype checks and consistent supervised row cleaning are implemented (R01–R08). Remaining JSON transactions, stronger input limits, broader runtime integration, finer token scopes and backup/restore are tracked in ROADMAP. Use a reverse proxy or external identity provider for production authentication.
